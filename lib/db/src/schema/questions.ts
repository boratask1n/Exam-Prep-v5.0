...
export const questionsTable = pgTable(
  "questions",
  {
    ...
    solutionYoutubeEndSecond: integer("solution_youtube_end_second"),
  ...
  (table) => ({
    ...
    resourceIdIdx: index("questions_resource_id_idx").on(table.resourceId),
    ...
    badgeFlagsIdx: index("questions_badge_flags_idx").on(
      table.isOsymBadge,
      table.isPremiumBadge,
    ),
  }),
);

export const questionReviewStatsTable = pgTable(
  ...
  (table) => ({
    nextEligibleIdx: index("question_review_stats_next_eligible_idx").on(
      table.nextEligibleAt,
    ),
    updatedAtIdx: index("question_review_stats_updated_at_idx").on(
      table.updatedAt,
    ),
    lastOutcomeIdx: index("question_review_stats_last_outcome_idx").on(
      table.lastOutcome,
    ),
  }),
);
