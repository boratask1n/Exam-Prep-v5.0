...
const navItems = [
  ...
  { href: "/practice-exams", icon: BookOpen, label: "Denemelerim" },
];
...
export function Sidebar({ children, userName, onLogout, onDeleteAccount }: SidebarProps) {
  ...
  const duckAudioBlobUrlRef = useRef<string | null>(null);
  ...
  useEffect(() => {
    return () => {
      ...
      if (duckAudioBlobUrlRef.current) {
        URL.revokeObjectURL(duckAudioBlobUrlRef.current);
        duckAudioBlobUrlRef.current = null;
      }
    };
  }, []);

  const playDuckQuack = async () => {
    const audioPath = "/brand/duck-quack.mp3?v=2";
    try {
      let blobUrl = duckAudioBlobUrlRef.current;
      ...
      if (!blobUrl) {
        ...
        duckAudioBlobUrlRef.current = blobUrl;
      }
      ...
      console.warn("Duck quack blob playback failed, trying fallback:", err);
      try {
        ...
        audio.play().catch((fallbackErr) => {
          console.error("Duck quack fallback playback failed:", fallbackErr);
          cleanup();
        });
      } catch (fallbackErr) {
        console.error("Duck quack fallback instantiation failed:", fallbackErr);
      }
    }
  };
