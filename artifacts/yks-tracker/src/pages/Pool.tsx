...
const QuestionCard = memo(function QuestionCard({
  ...
  const solutionVideoUrl = question.solutionYoutubeUrl || question.solutionUrl;
  const isYoutube = !!getYoutubeVideoId(solutionVideoUrl);
          ...
          isYoutube ? (
            <button
              ...
              onClick={(event) => {
                ...
                window.open(youtubeUrl || solutionVideoUrl, "_blank", "noopener,noreferrer");
              }}
            ...
            <button
              ...
              onClick={(event) => {
                event.stopPropagation();
                window.open(solutionVideoUrl, "_blank", "noopener,noreferrer");
              }}
              className="mb-2 inline-flex w-fit items-center gap-1.5 rounded-full border border-blue-300/50 bg-blue-500/10 px-2.5 py-1 text-[11px] font-medium text-blue-700 transition-colors hover:bg-blue-500/15 dark:border-blue-400/20 dark:text-blue-200"
            >
...
export default function Pool() {
  ...
  useEffect(() => {
    ...
    if (rId) {
      ...
      if (!Number.isNaN(parsed)) {
        newFilters.resourceId = parsed;
      }
    }
    if (tParam) {
      newFilters.topic = tParam;
    }
    if (newFilters.resourceId || newFilters.topic) {
  ...
  const { data: resourcesList = [] } = useListResources(
    {
    ...
    <PageShell>
      ...
      <PageSection className="backdrop-blur-none bg-white/90 dark:bg-slate-950/84">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Select
            value={filters.resourceId ? String(filters.resourceId) : "ALL"}
            onValueChange={(value) =>
              updateFilter({
                resourceId: value === "ALL" ? undefined : Number(value),
              })
            }
          >
            <SelectTrigger className="h-9 w-48 rounded-xl border-border/50 bg-background">
              <SelectValue placeholder="Kayıtlı Kaynak" />
            </SelectTrigger>
            <SelectContent>
