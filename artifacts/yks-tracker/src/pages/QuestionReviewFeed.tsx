...
import { BookOpenCheck, CheckCircle2, Clock3, Repeat2, RotateCcw, Search, XCircle, Youtube } from "lucide-react";
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-border/45 px-5 py-4 sm:px-7">
        <div className="space-y-2">
          <div className="flex flex-wrap items-center gap-2">
            <Badge variant="secondary" className="rounded-full bg-primary/10 px-2.5 py-1 text-[10px] uppercase tracking-[0.18em] text-primary">
              {question.category}
            </Badge>
            <Badge variant="secondary" className="rounded-full bg-foreground/6 px-2.5 py-1 text-[10px] uppercase tracking-[0.18em]">
              {question.lesson}
            </Badge>
            {question.hasDrawing ? (
              <Badge variant="secondary" className="rounded-full bg-violet-500/10 px-2.5 py-1 text-[10px] uppercase tracking-[0.18em] text-violet-600 dark:text-violet-200">
                Çizimli
              </Badge>
            ) : null}
          </div>
          ...
          <p className="text-sm text-muted-foreground">
            {[question.publisher, question.testName, question.testNo ? `Test ${question.testNo}` : null]
              .filter(Boolean)
              .join(" · ") || "Kaynak bilgisi yok"}
          </p>
        </div>
        <div className={cn("flex items-center gap-2 rounded-full bg-foreground/5 px-3 py-2 text-xs font-medium", meta.className)}>
      ...
      <div className="relative flex min-h-0 flex-1 items-center justify-center bg-white/56 p-4 dark:bg-white/[0.035] sm:p-6">
        {question.isOsymBadge || question.isPremiumBadge ? (
          <div className="pointer-events-none absolute right-5 top-5 z-10 flex flex-col items-end gap-2">
            ...
            {question.isPremiumBadge ? <QuestionBadgeImage type="premium" /> : null}
          </div>
        ) : null}
        ...
        {question.imageUrl ? (
          ...
          <div className="flex min-h-[28vh] w-full max-w-2xl items-center justify-center rounded-[1.4rem] border border-dashed border-border/70 bg-background/72 p-8 text-center text-muted-foreground">
            Bu soru için görsel yok. Açıklama ve meta veriler üzerinden tekrar edebilirsin.
          </div>
        )}
      </div>
      ...
      <div className="grid gap-3 border-t border-border/45 px-5 py-3 text-sm text-muted-foreground sm:grid-cols-[1fr_auto] sm:px-7">
        <div>
          <p className="line-clamp-2">{question.description || "Açıklama eklenmemiş."}</p>
          <div className="mt-2 flex flex-wrap items-center gap-2 text-xs">
            <span>Tekrar geçmişi: {reviewCount} değerlendirme · {correctCount} doğru · {wrongCount} tekrar</span>
            {youtubeUrl ? (
              <button
                type="button"
              ...
              >
                ...
                {question.solutionYoutubeStartSecond ? `${question.solutionYoutubeStartSecond}. saniyeden video çözüm` : "Video çözümü"}
              </button>
            ) : null}
          </div>
        </div>
        <div className="flex items-center gap-2 text-xs">
          <span className="rounded-full bg-foreground/5 px-3 py-1">Aşama {question.repetitionStage ?? 0}</span>
          <span className="rounded-full bg-foreground/5 px-3 py-1">{question.source}</span>
        </div>
      </div>
    </article>
  );
...
export default function QuestionReviewFeed() {
  ...
  const [algorithmText, setAlgorithmText] = useState("Yanlış ve zamanı gelen soruları öne alır.");
  ...
  const fetchBatch = useCallback(async (replace: boolean) => {
    ...
    try {
      const excludeIds = Array.from(new Set([...(replace ? [] : questionsRef.current.map((question) => question.id)), ...recentIdsRef.current.slice(-24)]));
      const query = new URLSearchParams({ limit: "8" });
      if (debouncedSearch.trim()) query.set("search", debouncedSearch.trim());
      if (excludeIds.length > 0) query.set("excludeIds", excludeIds.join(","));

      const response = await requestJson<FeedResponse>(`/api/questions/review/feed?${query.toString()}`);
      setAlgorithmText(response.algorithm?.description || "Yanlış ve zamanı gelen soruları öne alır.");
      ...
      setQuestions((current) => {
        ...
        return [...current, ...response.items.filter((question) => !existingIds.has(question.id))];
      });
    } finally {
  ...
  useEffect(() => {
    ...
    const targets = Array.from(root.querySelectorAll<HTMLElement>("[data-review-question-id]"));
  ...
  useEffect(() => {
    ...
    setFeedbackPending(null);
    recentIdsRef.current = [...recentIdsRef.current.filter((id) => id !== currentVisibleId), currentVisibleId].slice(-30);
    ...
    if (!lastServedAtRef.current[currentVisibleId] || now - lastServedAtRef.current[currentVisibleId] >= 45000) {
      ...
      void requestJson(`/api/questions/review/serve/${currentVisibleId}`, { method: "POST" });
    }

    const visibleIndex = questions.findIndex((question) => question.id === currentVisibleId);
  ...
  const sendFeedback = useCallback(async (feedback: Feedback) => {
    ...
    try {
      await requestJson(`/api/questions/review/feedback/${currentQuestion.id}`, {
        method: "POST",
      ...
      setQuestions((current) => current.filter((question) => question.id !== currentQuestion.id));
    } catch {
      setNotice({ tone: "error", text: "Geri bildirim kaydedilemedi, tekrar deneyebilirsin." });
    } finally {
      setFeedbackPending(null);
    }
  ...
  return (
    <PageShell maxWidthClassName="max-w-6xl" contentClassName="gap-4">
      <PageHeader
        ...
        actions={
          <Button variant="outline" className="rounded-xl" onClick={() => setReloadSeed((value) => value + 1)}>
      ...
      {isLoading ? (
        ...
        <div className="flex min-h-[55vh] flex-col items-center justify-center rounded-[1.6rem] border border-border/50 bg-card/72 p-8 text-center">
          ...
          <h2 className="mt-4 text-xl font-semibold">Tekrar akışı şimdilik boş</h2>
          <p className="mt-2 max-w-md text-sm text-muted-foreground">
            Soru ekleyip test çözdükçe ya da sorulara yanlış/doğru durum verdikçe burası otomatik dolacak.
          </p>
        </div>
      ) : (
        <div className="relative">
          ...
          >
            ...
            <div className="flex min-h-16 items-center justify-center py-4 text-sm text-muted-foreground">
              {isLoadingMore ? "Sıradaki sorular hazırlanıyor..." : hasMore ? "Kaydırmaya devam et, yeni sorular geliyor." : "Şimdilik tekrar kuyruğunun sonuna geldin."}
            </div>
          </div>
          ...
          {currentQuestion ? (
            <div className="pointer-events-none fixed bottom-5 right-5 z-30 sm:bottom-6 sm:right-6">
              <div className="pointer-events-auto flex flex-col items-end gap-2">
                <span className="rounded-full border border-border/70 bg-card/90 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.16em] text-muted-foreground shadow-[0_18px_50px_-30px_rgba(15,23,42,0.45)] backdrop-blur-xl">
                  Soru ayarı
                </span>
                <Button
                  type="button"
                  variant="outline"
                  className="h-10 rounded-full border-rose-300/70 bg-card/92 px-4 text-rose-700 shadow-[0_18px_50px_-30px_rgba(15,23,42,0.45)] backdrop-blur-xl hover:bg-rose-50/95 dark:border-rose-500/20 dark:text-rose-100 dark:hover:bg-rose-500/12"
                  onClick={() => void sendFeedback("again")}
                  disabled={feedbackPending !== null}
                >
                  {feedbackPending === "again" ? "Kaydediliyor..." : "Tekrar getir"}
                </Button>
                <Button
                  type="button"
                  variant="outline"
                  className="h-10 rounded-full border-emerald-300/70 bg-card/92 px-4 text-emerald-700 shadow-[0_18px_50px_-30px_rgba(15,23,42,0.45)] backdrop-blur-xl hover:bg-emerald-50/95 dark:border-emerald-500/20 dark:text-emerald-100 dark:hover:bg-emerald-500/12"
                ...
                <Button
                  type="button"
                  variant="outline"
                  className="h-10 rounded-full border-violet-300/70 bg-card/92 px-4 text-violet-800 shadow-[0_18px_50px_-30px_rgba(15,23,42,0.45)] backdrop-blur-xl hover:bg-violet-50/95 dark:border-violet-500/20 dark:text-violet-100 dark:hover:bg-violet-500/12"
                  onClick={() => void sendFeedback("less_often")}
                  disabled={feedbackPending !== null}
                >
                  {feedbackPending === "less_often" ? "Kaydediliyor..." : "Daha seyrek göster"}
                </Button>
                <Button
                  type="button"
                  variant="outline"
                  className="h-10 rounded-full border-sky-300/70 bg-card/92 px-4 text-sky-800 shadow-[0_18px_50px_-30px_rgba(15,23,42,0.45)] backdrop-blur-xl hover:bg-sky-50/95 dark:border-sky-500/20 dark:text-sky-100 dark:hover:bg-sky-500/12"
                  onClick={() => void sendFeedback("more_often")}
                  disabled={feedbackPending !== null}
                >
                  {feedbackPending === "more_often" ? "Kaydediliyor..." : "Daha sık göster"}
                </Button>
              </div>
            </div>
          ) : null}
