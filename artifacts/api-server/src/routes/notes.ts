...
router.get("/notes", async (req, res) => {
  ...
  if (req.query.resourceId) {
    const rId = Number.parseInt(req.query.resourceId as string, 10);
    if (!Number.isNaN(rId)) conditions.push(eq(notesTable.resourceId, rId));
  }
...
router.post("/notes", async (req, res) => {
  ...
  const inserted = (await db
    ...
    .values({
      ...
      userId,
      resourceId: req.body.resourceId ? Number.parseInt(req.body.resourceId, 10) || null : null,
      category,
      lesson,
      title,
...
router.patch("/notes/:id", async (req, res) => {
  ...
  const updatedRows = (await db
    .update(notesTable)
    .set({
      resourceId: req.body.resourceId === undefined ? existing.resourceId : (req.body.resourceId ? Number.parseInt(req.body.resourceId, 10) || null : null),
